    ...
    try:
        ...
        while order is None and pg <= total_pages:
            ...
            if resp.get("ec") != 200:
                break
            order = next(
                (o for o in resp.get("data", {}).get("list", []) if o.get("out_trade_no") == order_no),
                None,
            )
            pg += 1
        ...
        if order.get("status") != 2:
            ...
            return
        if await svc.storage.is_order_processed(order_no):
        ...
        if create_time and plan_days > 0:
            ...
            if datetime.now() > order_expire:
                yield event.plain_result(
                    f"该订单已超过赞助有效期（{order_expire.strftime('%Y-%m-%d')}），无法绑定"
                )
                return

        umo = event.unified_msg_origin
        user_id = order.get("user_id", "")
        existing = await svc.storage.get_user_mapping(user_id)
        if existing:
            existing_data = await svc.storage.get_umo_data(umo)
            if existing_data and order_no in existing_data.get("used_orders", []):
                await svc.storage.unmark_order_processed(order_no)
                yield event.plain_result("该订单已被使用")
                return

        # 先完成绑定，再标记订单已处理：绑定失败时订单不会被“吞”
        ...
        try:
            ...
            log_msg(svc.wire, f"标记订单 {order_no} 失败（绑定已生效，轮询会补齐）: {e}", "warning")
        ...
        log_msg(svc.wire, f"绑定成功: 用户{user_id} 订单{order_no} 方案Lv{plan.get('level')}")
        yield event.plain_result(
            f"✅ 绑定成功！\n\n"
        ...
        log_msg(svc.wire, f"绑定订单 {order_no} 失败: {e}", "error")
        yield event.plain_result("绑定失败，请稍后重试（已记录日志）")
...
async def cmd_models(svc: Services, event):
    ...
    umo_data = await svc.storage.get_umo_data(event.unified_msg_origin)
    level_label, model_id_map, all_models = _get_available_models(svc, umo_data)
    current = await svc.storage.get_current_model(event.unified_msg_origin)
...
async def cmd_switch(svc: Services, event, is_admin_fn):
    ...
    umo = event.unified_msg_origin
    umo_data = await svc.storage.get_umo_data(umo)
    ...
    try:
        ...
        await svc.astrbot_context.provider_manager.set_provider(
            model_name, ProviderType.CHAT_COMPLETION, umo
        )
    except ValueError as e:
        log_msg(svc.wire, f"切换模型失败: {model_name} 未配置对应提供商 ({e})", "warning")
        yield event.plain_result(
            f"切换失败：模型 {model_name} 未在 AstrBot 中配置对应提供商，请先添加"
        )
        return
    except Exception as e:
        log_msg(svc.wire, f"切换模型失败: {model_name} - {e}", "error")
        yield event.plain_result("切换失败，请稍后重试（已记录日志）")
        return

    await svc.storage.set_current_model(umo, model_name)
    log_msg(svc.wire, f"模型切换成功: umo={umo} -> {model_name}")
...
async def _is_group_admin(svc: Services, event) -> bool:
    """检查发送者是否为群主或群管。优先读取平台下发的 group 角色信息，再走静态管理员列表。"""
    try:
        sender_id = str(event.get_sender_id())
        group = getattr(event.message_obj, "group", None)

        if group and group.group_owner and sender_id == str(group.group_owner):
            return True
        if group and group.group_admins and sender_id in [str(a) for a in group.group_admins]:
            return True
    except Exception as e:
        log_msg(svc.wire, f"读取群角色信息异常: {e}", "warning")
    ...
    try:
        ...
        log_msg(svc.wire, f"读取静态管理员列表异常: {e}", "warning")
    return False
